...
module MarronageCertaintyRadius
  ...
  # Compute the mean radius of a circle polygon expressed as [[lon, lat], ...].
  # turf.circle() vertices are ~equidistant from the center; we average to
  # smooth out float drift.
  def polygon_radius_km(ring)
    ...
    center_lon = verts.map { |p| p[0] }.sum / verts.length.to_f
    center_lat = verts.map { |p| p[1] }.sum / verts.length.to_f

    distances = verts.map { |p| haversine_km(center_lon, center_lat, p[0], p[1]) }
  ...
  def radius_map_from_geojson(geojson_path)
    ...
    features.each do |feat|
      ...
      raw[settlement_id] << polygon_radius_km(ring)
    end
